...
function installACPartTwo(){
  ...
  if(!_hasXMLHTTP){
    // setCookie("qu","",0,_completeSearchString,null,null)
		_completetemp= "/crosslan/getsp.php";
		setCookie("qu","",0,_completetemp,null,null)
  }
  
  _documentForm.onsubmit=Fa; //commented by F
  ...
  _inputField.onsubmit=Fa; //commented by F
...
function onBlurHandler(event){
  ...
  if(!_cursorUpDownPressed){
    ...
    if(_lastKeyCode==9){
      setSubmitButtonFocus();  //commented by F
      _lastKeyCode=-1
    }
  }
...
function stripCRFromString(va){
  for(var f=0,oa="",zb="\n\r"; f<va.length; f++) {
	...
	// added by F ---->
	var temp2 = new RegExp("<b>","ig");	
	oa = oa.replace( temp2, "");
	temp2 = new RegExp("</b>","ig");
	oa = oa.replace( temp2, "");
	// <-------- added by F
...
function xb(eb) {
  ...
  if(eb=="url"){
    ...
    //_documentForm.submit(); //commentted by F
    return true //commentted by F
      ...
      if(ma){
        ...
        if(_hasXMLHTTP){
          callGoogle(Za)
				  		...
				  		_completetemp= "/crosslan/getsp.php";
							setCookie("qu",Za,0,_completetemp,null,null)
...
function callGoogle(Rb){
  ...
  if(_xmlHttp){
    ...
    // _xmlHttp.open("GET",_completeSearchEnString+"&js=true&qu="+Rb,true);
		...
		//_xmlHttp.open("GET","http://130.14.88.25/suggest/gets.php?qu="+Rb,true);
		_xmlHttp.open("GET","getsp.php?qu="+Rb,true);
    // Note that this function will ONLY be called when we get a complete
    // response back from google!!
    _xmlHttp.onreadystatechange=function() {
		  //alert("_xmlHttp1:"+_xmlHttp.responseText);
      if(_xmlHttp.readyState==4&&_xmlHttp.responseText) {
        var frameElement=B;
        if(_xmlHttp.responseText.charAt(0)=="<"){
				  //temp = _xmlHttp.responseText;
					alert("_xmlHttp:"+_xmlHttp.responseText);
          _timeoutAdjustment--
        }else{
          ...
          // eval(_xmlHttp.responseText);
					var temp = _xmlHttp.responseText;
					var temp_array = temp.split("new Array(");
					var temp2 = new RegExp(Rb,"ig");
					var temp3 = temp_array[1].replace(temp2, "<b>"+Rb+"</b>") ;
				
					var temp4 = temp_array[0]+"new Array("+temp3+"new Array("+temp_array[2]+"new Array("+temp_array[3];
		
					eval(temp4);
					// added by F <-----
					
					//sendRPCDone(frameElement, "clinical", new Array("clinical depression", "clinical trials", "clinical governance", "clinical psychology", "clinical research", "clinical pharmacology", "clinical evidence", "clinical psychologist", "clinical audit", "clinical chemistry"), new Array("5,060,000 results", "10,400,000 results", "976,000 results", "5,720,000 results", "11,800,000 results", "3,090,000 results", "9,070,000 results", "931,000 results", "1,470,000 results", "3,880,000 results"), new Array("")); 

					//alert("done?"+eval(_xmlHttp.responseText));										
        }
      }
    }
    ;
